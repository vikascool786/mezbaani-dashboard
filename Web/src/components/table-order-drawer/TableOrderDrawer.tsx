...
const TableOrderDrawer: React.FC<Props> = ({ table, isOpen, onClose, onOrderUpdated }) => {
  ...
  ------------------------- */
    ...
    const getDraftQty = (menuItemId: string) => {
        return (
            draftItems.find(i => i.menuItemId === menuItemId)?.quantity || 0
        );
    };
    ...
    const onPrintKot = async () => {
        ...
        try {
            ...
            if (!orderId) {
                const res = await apiCall(`${baseUrl}/orders`, {
                    method: "POST",
                    body: JSON.stringify({
                        tableId: table.id,
                        items: itemsPayload,
                ...
                orderId = res.id;
                setOrder(res); // set newly created order
            }

            // CASE 2: OCCUPIED TABLE
            else {
                await apiCall(`${baseUrl}/orders/${orderId}`, {
                    method: "PUT",
                    body: JSON.stringify({
                        tableId: table.id,
                        items: itemsPayload,
    ...
    const confirmCloseBill = async () => {
        ...
        try {
            ...
            await apiCall(
                `${baseUrl}/bill/${order.id}/generate`,
                {
                    method: "POST",
                }
            );
            await apiCall(
                `${baseUrl}/payment/${order.id}/pay`,
                {
                    method: "POST",
                    body: JSON.stringify({
    ...
    return (
        <>
            ...
            <div className="drawer-header">
                <TableHeader table={table} order={order} onClose={onClose} showCloseBill={showCloseBill} />
            </div>
